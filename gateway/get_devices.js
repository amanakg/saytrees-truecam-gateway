...
if (process.argv.length < 4) {
  console.log('Usage: node get_devices.js <account> <password>');
  console.log('Example: node get_devices.js info@enarxi.com Enarxi12345@');
  process.exit(1);
}
...
async function run() {
  try {
    ...
    console.log(`\nFound ${deviceList.length} device(s) in account:\n`);
    ...
    deviceList.forEach((device, index) => {
      ...
      console.log(`Device UUID:   ${params.deviceUuid}`);
